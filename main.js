...
const noEventMessage = $eventContainer.data('no-event');
...
function isHavingEvent(date) {
    ...
    allEvents.forEach(element => {
        const eleTimeStamp = moment(element.day, 'dd-mm-yyyy');
        if ((new Date(dateTimeStamp)).getTime() === (new Date(eleTimeStamp)).getTime()) {
            hasEvent = true;
        }
    });
